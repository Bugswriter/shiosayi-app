...
<footer
  class="fixed bottom-0 left-0 right-0 flex items-center justify-between bg-gray-100 px-4 py-1 border-t border-gray-300 text-sm text-gray-600 shadow-inner z-10 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400"
>
  ...
  <div class="flex items-center gap-2">
    <!-- Guardian Info Button (conditional) -->
    {#if $guardian}
      <button
        on:click={() => (showGuardianInfo = true)}
        aria-label="Open guardian profile"
        class="px-3 py-1.5 flex items-center gap-2 rounded-full text-gray-600 bg-gray-200 hover:bg-gray-300 dark:text-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 transition-colors"
      >
        <svg
        ...
        <span class="text-xs font-medium">{$guardian.name}</span>
      </button>
    {/if}
    ...
    <button
      ...
      class="p-2 rounded-full text-gray-500 hover:bg-gray-200 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white transition-colors"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
